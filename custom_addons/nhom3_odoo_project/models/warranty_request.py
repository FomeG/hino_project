...
class WarrantyRequest(models.Model):
    ...
    x_owner_name=fields.Many2one( 'res.partner',related='x_repair_order.partner_id',string="Owner vehicle name", readonly=True)
    x_address=fields.Char(related='x_repair_order.x_vehicle_plate.address',string="Address", readonly=True)
    x_tel=fields.Char(related='x_repair_order.x_vehicle_plate.contact_phone_2',string="Phone", readonly=True)
    x_driver=fields.Many2one(related='x_repair_order.x_vehicle_plate.driver',string="Driver", readonly=True)
    x_driver_phone=fields.Char(related='x_repair_order.x_vehicle_plate.driver_phone',string="Phone", readonly=True)
    ...
    def create(self, vals):
        ...
        # Lấy số bảo hành mới nhất trong năm hiện tại
        last_request = self.search([('x_warranty_request_number', 'like', f'CB/{current_year}%')], 
                                   order='x_warranty_request_number desc', limit=1)
        
        # Nếu có số bảo hành, lấy số tự động tiếp theo
        if last_request:
            last_number = int(last_request.x_warranty_request_number.split('/')[-1])
            new_number = str(last_number + 1).zfill(4)  # Tăng số tự động và giữ đúng 4 chữ số
        else:
            new_number = '0001'  # Nếu không có số bảo hành nào, bắt đầu từ 0001

        # Tạo số bảo hành mới với định dạng CB/year/0001
        warranty_number = f'CB/{current_year}/{new_number}'

        # Thêm số bảo hành vào các giá trị trước khi tạo bản ghi
        vals['x_warranty_request_number'] = warranty_number

        # Gọi hàm create để tạo bản ghi
